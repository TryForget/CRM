...
public class UserService extends BaseService<User, Integer> {
     ...
     * @param userId
     */
    public void updateUserPassWord(String oldPwd, String newPwd, String repeatPwd, Integer userId) {
        // 1. 调用Dao层，通过用户ID得到用户对象
        User user = userMapper.selectByPrimaryKey(userId);
        // 2. 判断用户对象是否存在
        AssertUtil.isTrue(null == user, "待更新用户不存在！");
        // 3. 判断参数是否为空 且 满足需求
        checkUpdatePwdParams(oldPwd, newPwd, repeatPwd, user.getUserPwd());
        // 4. 通过用户ID修改用户密码，判断受影响的行数
     ...
     * @param userPwd
     */
    private void checkUpdatePwdParams(String oldPwd, String newPwd, String repeatPwd, String userPwd) {
        // 原始密码是否为空
        AssertUtil.isTrue(StringUtils.isBlank(oldPwd), "原始密码不能为空！");
        // 原始密码是否与数据库中的用户对象的密码一致 （加密后再比较）
        String pwd = Md5Util.encode(oldPwd); // 加密后的密码
